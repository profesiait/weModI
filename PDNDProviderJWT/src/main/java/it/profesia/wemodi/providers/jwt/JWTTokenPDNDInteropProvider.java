...
public class JWTTokenPDNDInteropProvider {
    ...
    /**
     * Recupero del Voucher di interoperabilità (access token JWT) da PDND tramite JWT Assertion
     * 
     * @return Voucher PDND di interoperabilità (access token)
    ...
    public String PDNDJwtAssertion() throws OAuthSystemException {
        ...
        try(PEMParser pemParser = new PEMParser(new StringReader(pdndPKMapping.getPrivkey())))
        {
            ...
            String msg = "Errore nel recupero del voucher PDND di interoperabilità.";
            log.error(msg, e);
    ...
    /**
     * Invoca l'endpoint PDND per ottenere il Voucher di interoperabilità (access token)
    ...
    private String GetPDNDVoucher(CloseableHttpClient httpClient, HttpRequestBase httpRequest) throws OAuthSystemException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(httpRequest)) {
            ...
            log.debug("PDND access token di interoperabilità: " + voucher);
            
        } catch (ParseException | APISecurityException | UnsupportedOperationException | IOException e) {
            String msg = "Impossibile recuperare il voucher PDND di interoperabilità.";
            log.error(msg, e);
            throw new OAuthSystemException(msg, e);
        } finally {
            httpRequest.releaseConnection();
