...
public class JWTValidator {
	...
	public static Pair<String, Boolean> validatePdndSignature(String pdndjwt, String pdndJwksUrl, String pdnd_api_url, JWTInfo jwtInfo) 
	{
		...
		if (getModiCacheEnable() && getModiCache(jwtInfo.getApiName()).get(jwtInfo.getPdndKid()) != null) {
    		publicKey = (PublicKey) getModiCache(jwtInfo.getApiName()).get(jwtInfo.getPdndKid());
        ...
        else
        {
        	...
        	if(!(pdndAccessToken.equals("")))
        	{
    			...
    			content = callExternalUrl(pdnd_api_url+"/keys/"+jwtInfo.getPdndKid(), pdndAccessToken);
        		publicKey = retrievePubKeyFromJWKS(content, "");
        		if(getModiCacheEnable())
        		{
        			getModiCache(jwtInfo.getApiName()).put(jwtInfo.getPdndKid(), publicKey);
	...
	private static String retrieveAccessTokenPdndApiInterop(JWTInfo jwtInfo) {
		String pdndAccessToken = "", applicationUUID = "";
		try {
			applicationUUID = jwtInfo.getApplicationUUID();
			log.info("applicationUUID: " + applicationUUID + " iss: " + jwtInfo.getPdndIss());
			if (!applicationUUID.equals("") && (jwtInfo.getPdndIss() == null || !(jwtInfo.getPdndIss().contains("pagopa")))) {
