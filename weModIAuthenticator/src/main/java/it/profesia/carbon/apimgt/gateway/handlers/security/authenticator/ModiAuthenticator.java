...
import it.profesia.wemodi.providers.jwt.JWTTokenPDNDInteropProvider;
...
public class ModiAuthenticator implements Authenticator {
    ...
    public AuthenticationResponse authenticate(MessageContext synCtx) {
        ...
        if (retrievedApi != null) {
            ...
            try {
            	...
            	String contentType = "", digest = "", serverUrl = "", /*requestURI = "",*/ aud = "", subUUID = "";
            	...
            	jwtInfo.setApiName(apiName.split(":")[0]);
            	jwtInfo.setApplicationUUID(subUUID);
                ...
                if(isValid)
                {
                    ...
                    if(jwtInfo.getSub() != null && !(jwtInfo.getSub().equals("")))
                		{
                			log.info("Il claim sub Ã¨ valorizzato: " + jwtInfo.getSub());
                			...
                			if (datastore != null) {
                            	...
                            	if(app != null)
                            	{
                            		...
                            		if(appId != 0)
                        			{
	                            		/*https://github.com/wso2/api-manager/issues/3136
	                            		apiKeyValidationInfoDTO = getApiKeyValidator().validateSubscription(apiContext, apiVersion, appId, apiTenantDomain);
	                            		*/
                            			String keyManager = "", consumerKey = "";
                            			List<ApplicationKeyMapping> appKeyMappingList = datastore.getKeyMappingByApplicationId(appId);
                            			for(ApplicationKeyMapping appKeyMapping : appKeyMappingList)
                            			{
                            				keyManager = appKeyMapping.getKeyManager();
                            				consumerKey = appKeyMapping.getConsumerKey();
                            				log.debug("Key Manager:" + keyManager + " " + consumerKey);
                            				if(appKeyMapping.getKeyType().equals(APIConstants.API_KEY_TYPE_PRODUCTION))
                            					break;
                            			}
	                        			//validate subscription
	                            		apiKeyValidationInfoDTO = getApiKeyValidator().validateSubscription(apiContext, apiVersion, consumerKey, apiTenantDomain, keyManager);
	                            		//end validate
            			...
            			log.info("Il claim sub non Ã¨ valorizzato");
            		...
            		{
            			log.info("Subscription validation failed: it could an automatic authorization");
            		...
            		{
            			log.error("Cert app mapping validation failed or PDND client api interop validation");
            			JWTClaimsSet payload;
                        SignedJWT signedJWT;
                        String tokenIdentifier = "", pdndAccessTokenInterop = "", client = "", agreement = "", eservice = "";
                        
                        String applicationUUID = subUUID;
        				
                        if (getModiCacheEnable() && getModiCache(jwtInfo.getApiName()).get(applicationUUID) != null) {
                        	pdndAccessTokenInterop = (String) getModiCache(jwtInfo.getApiName()).get(applicationUUID);
                    		log.debug("PDND token interop FOUND in cache");
                    		if(isJwtTokenExpired(pdndAccessTokenInterop))
                    		{
                    			pdndAccessTokenInterop = getPDNDVoucherInterop(applicationUUID);
                            	if(getModiCacheEnable())
                            	{
                            		getModiCache(jwtInfo.getApiName()).put(applicationUUID, pdndAccessTokenInterop);
                            		log.debug("PDND token interop PUT in cache because of expired");
                            	}
                    		}
                    	}
                        else
                        {
                        	...
                        	{
                        		getModiCache(jwtInfo.getApiName()).put(applicationUUID, pdndAccessTokenInterop);
                        		log.debug("PDND token interop PUT in cache");
                        	}
                        }
        				try
        				{
        					if (getModiCacheEnable() && getModiCache(jwtInfo.getApiName()).get(jwtInfo.getPdndClientId()) != null) {
        						client = (String)getModiCache(jwtInfo.getApiName()).get(jwtInfo.getPdndClientId());
        						log.debug("client FOUND in cache");
        					}
        					else
        					{
        						client = JWTValidator.callExternalUrl(pdnd_api_url+"/clients/"+jwtInfo.getPdndClientId(), pdndAccessTokenInterop);
                				if(getModiCacheEnable() && !(client.equals("")))
                            	{
                            		getModiCache(jwtInfo.getApiName()).put(jwtInfo.getPdndClientId(), client);
                            		log.debug("client PUT in cache");
                            	}
        					}
        					log.debug("client: " + client);
        					JsonObject clientJSON = new Gson().fromJson(client, JsonObject.class);
        					String consumerIdFromClient = clientJSON.get("consumerId").getAsString();
        					
        					if (getModiCacheEnable() && getModiCache(jwtInfo.getApiName()).get(jwtInfo.getPdndPurposeId()) != null) {
        						agreement = (String)getModiCache(jwtInfo.getApiName()).get(jwtInfo.getPdndPurposeId());
        						log.debug("agreement FOUND in cache");
        					}
        					else
        					{
        						agreement = JWTValidator.callExternalUrl(pdnd_api_url+"/purposes/"+jwtInfo.getPdndPurposeId()+"/agreement", pdndAccessTokenInterop);
                				if(getModiCacheEnable() && !(agreement.equals("")))
                            	{
                            		getModiCache(jwtInfo.getApiName()).put(jwtInfo.getPdndPurposeId(), agreement);
                            		log.debug("agreement PUT in cache");
                            	}
        					}
        					log.debug("agreement: " + agreement);
        					
        				JsonObject agreementJSON = new Gson().fromJson(agreement, JsonObject.class);
        				String eserviceId = agreementJSON.get("eserviceId").getAsString();
        				String descriptorId = agreementJSON.get("descriptorId").getAsString();
        				String consumerIdFromAgreement = agreementJSON.get("consumerId").getAsString();
        				String producerIdFromAgreement = agreementJSON.get("producerId").getAsString();
        				String stateFromAgreement = agreementJSON.get("state").getAsString();
        				
        				SignedJWT signedJWTTokenInterop = SignedJWT.parse(pdndAccessTokenInterop);
    					JWTClaimsSet payloadJWTTokenInterop = signedJWTTokenInterop.getJWTClaimsSet();
    					String organizationId = payloadJWTTokenInterop.getStringClaim("organizationId");
    					log.debug("organizationId: " + organizationId);
        				
        				if(consumerIdFromClient.equals(consumerIdFromAgreement) && organizationId.equals(producerIdFromAgreement) && stateFromAgreement.equals("ACTIVE"))
        				{
        					if (getModiCacheEnable() && getModiCache(jwtInfo.getApiName()).get(eserviceId) != null) {
            					eservice = (String)getModiCache(jwtInfo.getApiName()).get(eserviceId);
        						log.debug("eservice FOUND in cache");
        					}
        					else
        					{
        						eservice = JWTValidator.callExternalUrl(pdnd_api_url+"/eservices/"+eserviceId+"/descriptors/"+descriptorId, pdndAccessTokenInterop);
                				if(getModiCacheEnable() && !(eservice.equals("")))
                            	{
                            		getModiCache(jwtInfo.getApiName()).put(eserviceId, eservice);
                            		log.debug("eservice PUT in cache");
                            	}
        					}
            				log.debug("eservice: " + eservice);
            				
            				JsonObject eserviceJSON = new Gson().fromJson(eservice, JsonObject.class);
            				String audience = eserviceJSON.get("audience").getAsString();
            				
            				if(audience.equals(jwtInfo.getPdndAud()))
            				{
            					signedJWT = SignedJWT.parse(pdndJwt);
                                payload = signedJWT.getJWTClaimsSet();
                                tokenIdentifier = payload.getJWTID();
                                JSONObject apiJSON = new net.minidev.json.JSONObject();
                                apiJSON.put("name", retrievedApi.getApiName());
                                apiJSON.put("publisher", retrievedApi.getApiProvider());
                				AuthenticationContext authenticationContext = GatewayUtils
                                        .generateAuthenticationContext(tokenIdentifier, payload, apiJSON, retrievedApi.getApiTier());
                				synCtx.setProperty(APIMgtGatewayConstants.END_USER_NAME, authenticationContext.getUsername());
                                APISecurityUtils.setAuthenticationContext(synCtx, authenticationContext);
                				log.info("Erogazione autorizzata tramite le api di interoperabilità di PDND");
                                return new AuthenticationResponse(true, true, false, 0, null);
            				}
            				else
            				{
            					log.error("PDND audience non valido: " + audience + " - " + jwtInfo.getPdndAud());
                                throw new APISecurityException(APISecurityConstants.API_AUTH_INVALID_CREDENTIALS,
                                        APISecurityConstants.API_AUTH_INVALID_CREDENTIALS_MESSAGE);
            				}
        				}
        				...
        				{
        					log.error("PDND clientId | organizationId non validi o fruizione non attiva: " + consumerIdFromClient + " - " + consumerIdFromAgreement + " | " + organizationId + " - " + producerIdFromAgreement + " | " + stateFromAgreement);
                            throw new APISecurityException(APISecurityConstants.API_AUTH_INVALID_CREDENTIALS,
                                    APISecurityConstants.API_AUTH_INVALID_CREDENTIALS_MESSAGE);
        				}
        				}
	...
	private AuthenticationContext generateAuthenticationContext(APIKeyValidationInfoDTO apiKeyValidationInfoDTO, String endUserToken) {
		...
		if (apiKeyValidationInfoDTO != null) {
			...
			if(!(apiKeyValidationInfoDTO.getType().equals("SANDBOX")) && !(apiKeyValidationInfoDTO.getType().equals("PRODUCTION")))
				apiKeyValidationInfoDTO.setType(APIConstants.API_KEY_TYPE_PRODUCTION);
				
			log.debug("key type: " + apiKeyValidationInfoDTO.getType());
	...
	private String getPDNDVoucherInterop(String applicationUUID) throws APIManagementException, APISecurityException, URISyntaxException
	{
		String pdndAccessTokenInterop = "";
		JWTTokenPDNDInteropProvider jwtTokenPDNDInteropProvider = null;
		jwtTokenPDNDInteropProvider = JWTTokenPDNDInteropProvider.FromApplicationUUID(applicationUUID);
		try {
			pdndAccessTokenInterop = jwtTokenPDNDInteropProvider.PDNDJwtAssertion();
		} catch (OAuthSystemException e) {
			log.error("Impossibile recuperare le informazioni per il token PDND di interoperabilità.", e);
			throw new APISecurityException(APISecurityConstants.API_AUTH_INVALID_CREDENTIALS,
                    APISecurityConstants.API_AUTH_INVALID_CREDENTIALS_MESSAGE);
		}
		return pdndAccessTokenInterop;
	}
	
	private String getApplicationUUIDByName(SubscriptionDataStore datastore, String apiTenantDomain)
	{
		List<Application> applications = null;
		String applicationUUID = "";
		if (datastore != null) {
            
        	applications = datastore.getApplicationsByName("ClientApiInteropApp");
            
            if (applications != null && applications.size() > 0) {
                for(Application application : applications) {
                	applicationUUID = application.getUUID();
                	log.info("applicationUUID for client api interop: " + applicationUUID);
                }
                
            } else {
                if (log.isDebugEnabled()) {
                    log.debug("Application not found in the datastore");
                }
            }
        } else {
            log.error("Subscription datastore is not initialized for tenant domain " + apiTenantDomain);
     ...
     */
    private boolean isJwtTokenExpired(String jwt) throws ParseException {
        ...
        if (!DateUtils.isAfter(exp, now, 0)) {
	...
	private static Cache getModiCache(String cacheName) {

        return CacheProviderWeModi.getWeModiCache(cacheName);
    }
