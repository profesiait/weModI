...
public class WeModIMediator extends AbstractMediator implements ManagedLifecycle {
	...
	public boolean mediate(MessageContext synCtx) {
		...
		try {
            ...
            if (getWeModiCacheEnable()) {
                Object cached = getWeModiCache(apiFromContext.getApiName()).get(pdndPKCacheKey);
                if (cached != null) {
                    pdndPK = (PdndPKMapping)cached;
                    log.debug("Cache enabled - pdndPK cached");
                }
            }
            ...
            if (pdndPK == null) {
                ...
                if (getWeModiCacheEnable()) {
                    getWeModiCache(apiFromContext.getApiName()).put(pdndPKCacheKey, pdndPK);
                    log.debug("Cache enabled - pdndPK not cached");
                }
            }
            ...
            if (apiConfig.isJwsAudit()) {
                ...
                if (getWeModiCacheEnable()) {
                    Object cached = getWeModiCache(apiFromContext.getApiName()).get(cacheKey);
                    if (cached != null) {
                        jwsAuditProvider = (JWSAuditProvider)cached;
                        log.debug("Cache enabled - jwsAuditProvider cached");
                    }
                }
                if (jwsAuditProvider == null) {
                    jwsAuditProvider = JWSAuditProvider.FromConsumerKey(consumerKey);
                    if (getWeModiCacheEnable()) {
                        getWeModiCache(apiFromContext.getApiName()).put(cacheKey, jwsAuditProvider);
                        log.debug("Cache enabled - jwsAuditProvider not cached");
                    }
                }
                ...
                try {
                    ...
                    if (getWeModiCacheEnable()) {
                        ...
                        if (cached != null) {
                            ...
                            log.debug("Cache enabled - jwtTokenPDNDProvider cached");
                        }
                    }
                    if (jwtTokenPDNDProvider == null) {
                        jwtTokenPDNDProvider = JWTTokenPDNDProvider.FromConsumerKey(consumerKey);
                        if (getWeModiCacheEnable()) {
                            getWeModiCache(apiFromContext.getApiName()).put(cacheKey, jwtTokenPDNDProvider);
                            log.debug("Cache enabled - jwtTokenPDNDProvider not cached");
                        }
                    }
                ...
                try {
                    ...
                    if (getWeModiCacheEnable()) {
                        ...
                        if (cached != null) {
                            ...
                            log.debug("Cache enabled - jwtTokenModIProvider cached");
                        }
                    }
                    if (jwtTokenModIProvider == null) {
                    	jwtTokenModIProvider = JWTTokenModIProvider.FromConsumerKey(consumerKey);
                        if (getWeModiCacheEnable()) {
                            getWeModiCache(apiFromContext.getApiName()).put(cacheKey, jwtTokenModIProvider);
                            log.debug("Cache enabled - jwtTokenModIProvider not cached");
                        }
                    }
                    ...
                    String modiTokenHeaderName = ((modiTokenHeaderName = apiConfig.getModiTokenName()) != null && !(apiConfig.getModiTokenName().equals(""))) ? modiTokenHeaderName : "Agid-JWT-Signature";
    ...
    private static Cache getWeModiCache(String cacheName) {
	...
	private void handleCacheConfig() {
		boolean enabledCache = false;
		long expiryTimeInSecondsCache = 0;
		String apiName = "";
		try {
			JsonObject properties = new Gson().fromJson(customParameters, JsonObject.class);
			if(properties.has("apiName"))
				apiName = properties.get("apiName").getAsString();
			...
			expiryTimeInSecondsCache = Long
					.parseLong(cacheConfigurations.get("expiryTimeInSecondsCache").getAsString());
			CacheProviderWeModi.setExpiryTimeInSecondsCache(expiryTimeInSecondsCache);

			if (CacheProviderWeModi.isEnabledCache() && !(apiName.equals(""))) {
