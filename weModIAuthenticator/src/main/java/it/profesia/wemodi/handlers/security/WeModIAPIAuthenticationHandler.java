...
public class WeModIAPIAuthenticationHandler extends APIAuthenticationHandler {
	...
	public static final String PUBLISHER_API = "/api/am/publisher";
    ...
    private ApiConfig retrieveAPIInformations(String apiID, String tenantDomain) throws DataLoadingException, IOException {
        ...
        String serviceURLStr = getEventHubConfigurationDto.getServiceUrl().concat(PUBLISHER_API);
                ...
                try {
                    ...
                    if (retryCount < retrievalRetries) {
                        ...
                        try {
                            ...
                            log.warn(String.format("Si è verificato un problema durante il recupero delle informazioni per l'API %s : %s", apiConfigKey, e.getLocalizedMessage()));
                        }
                    } else {
                        log.error(String.format("Impossibile recuperare le informazioni per l'API %s : %s", apiConfigKey, ex.getLocalizedMessage()));
                        throw ex;
                    }
                }
            } while (retry);
